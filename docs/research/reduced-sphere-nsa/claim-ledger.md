...
| `N-05` | `NSA-EQUIV` | Hyperfinite path sums represent length only when the partition resolves the active corrugation frequency. | hyperfinite integration in `NELSON`/`KEISLER` + paper frequency ladder | Derived scale condition: an infinite sample count can still alias a more-infinite frequency. |
...
| `C-03` | `NEW-COROLLARY` | Internal curve length is preserved while the ordinary length of the pointwise shadow is zero. | `C-01`, transferred length identity | Proved in the report. Standard part and length formation do not commute in this regime. |
| `C-04` | `NEW-COROLLARY` | The derivative cannot be S-continuous at any standard point. | `C-02`, exact internal metric, standard-part differentiation theorem | Proved by contradiction. The internal map may still be `*C1`; S-`C1` is the stronger property that fails. |
